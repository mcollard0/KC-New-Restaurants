...
class KCRestaurant:
    ...
    def enrich_restaurant_data( self, document: dict ) -> dict:
        ...
        try:
            with error_context( "google_places_enrichment", { 'restaurant': search_name, 'address': address } ):
                logger.debug( f"Enriching data for: {search_name} at {address}" );
                
                # Search for the place and get enriched data
                place_data = self.google_places_client.search_and_enrich_restaurant( search_name, address );
                ...
                if place_data:
                    ...
                    document.update( {
                        ...
                        'google_phone_number': place_data.phone_number,
                        'google_website': place_data.website,
                        'google_photo_urls': place_data.photo_urls,
